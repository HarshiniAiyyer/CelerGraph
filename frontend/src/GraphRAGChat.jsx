...
/**
 * MOCK DATA & CONSTANTS
 * Used for initial history. Graph nodes are now dynamic.
 */
const INITIAL_HISTORY = [];

/**
 * COMPONENT: Grainy Background
 * Adds that specific "noise" texture seen in the screenshot
 */
const NoiseOverlay = () => (
 ...
 * Renders dynamic nodes passed from the backend
 */
...
const ChatInterface = ({ onLogout }) => {
  ...
  const [messages, setMessages] = useState([
    { 
      id: 'welcome', 
      role: 'assistant', 
      content: "Hello! How can I help you? I can answer questions about the FastAPI codebase.", 
      hasGraph: false,
      graphNodes: []
    }
  ]);
   ...
   */
  // Temporary test function to verify history update works

    

  const handleSend = async () => {
    const updateHistory = (query, references) => {
        const newHistoryItem = { id: Date.now(), title: query, references };
    ...
    try {
      ...
      // 4. Save history after complete reply
      ...
      try {
        ...
        const h = await fetch(`${API_BASE}/history`);
        if (h.ok) {
      ...
      // Optional: keep a small static fallback removed for production
      
    } finally {
      setIsTyping(false);
      console.log('handleSend: Function finished.');
    }
  };
...
/**
 * MAIN APP CONTAINER
 * Manages the routing state between Landing Page and Chat App
 */
export default function CelerGraphApp() {
